...
import { LawyerProfileErrors, LawyerProfileInput, parseCommaSeparatedList, validateLawyerProfile } from './lawyerProfileValidation';
...
export const LawyerProfileView: React.FC = () => {
  ...
  const [practiceAreasText, setPracticeAreasText] = useState((profile?.practiceAreas || []).join(', '));
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const normalizedForm = {
      ...form,
      practiceAreas: parseCommaSeparatedList(practiceAreasText),
      languages: parseCommaSeparatedList(languagesText),
    };
    const nextErrors = validateLawyerProfile(normalizedForm);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length) return;
    setForm(normalizedForm);
    setPracticeAreasText(normalizedForm.practiceAreas.join(', '));
    setLanguagesText(normalizedForm.languages.join(', '));
    setIsSaving(true);
    setMessage('');
    try {
      const result = await apiRequest<{ lawyer: Lawyer; user: User }>('/api/lawyer/profile', { method: 'PUT', body: JSON.stringify(normalizedForm) });
      setUser(result.user);
  ...
  return <div className="h-full overflow-y-auto p-4 sm:p-6"><form onSubmit={submit} className="mx-auto max-w-4xl space-y-6">
    <div className="rounded-2xl border border-[#eadbc1] bg-white p-4 shadow-sm sm:p-6"><div className="flex items-start gap-4"><UserCircle className="h-12 w-12 text-[#701f2f]" /><div><h1 className="text-2xl font-bold text-[#32151b]">Professional profile</h1><p className="mt-1 text-sm text-[#6f5a49]">This information appears to clients after administrator approval.</p><span className="mt-3 inline-flex items-center gap-1 rounded-full bg-[#fff4d6] px-3 py-1 text-xs font-semibold uppercase text-[#765116]"><BadgeCheck className="h-4 w-4" />{status}</span>{profile?.rejectionReason && <p className="mt-3 text-sm text-red-700">Review note: {profile.rejectionReason}</p>}</div></div></div>
    <section className="grid gap-5 rounded-2xl border border-[#eadbc1] bg-white p-4 shadow-sm sm:p-6 md:grid-cols-2">
      ...
      <label className="text-sm font-medium">Practice areas (comma separated)<input className={inputClass} value={practiceAreasText} onChange={(e) => setPracticeAreasText(e.target.value)} placeholder="Family Law, Property Law" />{errors.practiceAreas && <small className="text-red-600">{errors.practiceAreas}</small>}</label>
      <label className="text-sm font-medium">Languages (comma separated)<input className={inputClass} value={languagesText} onChange={(e) => setLanguagesText(e.target.value)} placeholder="English, Hindi, Tamil" />{errors.languages && <small className="text-red-600">{errors.languages}</small>}</label>
